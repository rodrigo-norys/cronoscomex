...
| `sort` | `string` | `sourceRow` | `sourceRow` · `ref` · `client` · `clientProcess` · `importer` · `vessel` · `eta2` · `registrationDate` · `billOfLading` · `container` · `status` — dez por coluna da tabela, mais `sourceRow`, que **não tem coluna**: é a ordem física do arquivo, e é o padrão desde `H-89` (`D-33`). O padrão era `eta2` até 10/09/2026. `client` ordena pelo **cliente consolidado** e `clientProcess` pelo valor da célula CLT: são ordens diferentes desde `H-49`. `status` ordena pelo **fluxo** (`em_andamento` → `em_desembaraco` → `desembaracado` → `fechado_aguardando_draft`), não pelo alfabeto, e nunca tem valor ausente |
