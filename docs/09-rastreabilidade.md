...
| IND-18 | Ranking de clientes | CLT | Top 10 de IND-10, apresentação visual (A-25) | H-11, H-18, H-49 | `indicators-rankings.test.ts` · `indicators.test.ts` · `Clients.test.tsx` | ✅ **Entregue** — backend em `H-11`, barras horizontais em `H-18`. O corte vem de `meta.topN`, e a página o anuncia em vez de fixar 10. Desde `H-49` o rótulo de cada barra é o `label` do mapa, e não a referência de um processo |
...
| CLT | IND-10, IND-18, filtros `client` e `clientProcess` | H-03, H-11, H-49 | ✅ Implementável. Desde `H-49` a célula produz **dois** campos: o cliente consolidado e o processo daquele cliente (TD-04.1) |
