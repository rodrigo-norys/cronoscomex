...
| H-44 | §3.1 (as sete páginas), §3.1 (Página Histórico) | ✅ **Concluída.** Fecha `ACHADO 11` nas sete páginas e em `WorkbookSetup`: o `return` antecipado deixa de montar região já populada, e o anúncio passa pela região viva da casca, por portal — um nó que **sobrevive** à troca de estado da página, que é a condição de o leitor de tela ter o que comparar. O bloco visível fica `aria-hidden`, para o texto ser lido uma vez só. Fecha `ACHADO 12` com `accessibilityLayer={false}`: o Recharts dava `tabIndex=0` e `role="application"` ao `<svg>` dentro de subárvore `aria-hidden`, uma parada de tabulação sem nome acessível. E `ACHADO 13` pela **espessura** da borda, porque sob `forced-colors: active` a cor é substituída e a espessura não. Corrige de passagem um defeito de `H-43`: a região da casca tinha nascido sem `role` |
| H-45 | §3.1 (as sete páginas), §3.1 (Página Alertas), §3.1 (Página Inicial) | ✅ **Concluída.** Fecha `ACHADO 15`, `16` e `18`. O primeiro já vinha satisfeito — `H-42` alinhou os quatro desviantes de passagem ao migrar para tokens —, e o trabalho foi **travá-lo**: `tests/repo/estilo.test.ts` ganhou `C04`, com duas asserções de conjunto e prova por mutação. `ACHADO 16`: abrir o detalhe era `<a href>` na tabela e `<button>` no alerta, com dois nomes acessíveis para a mesma ação — `SC 3.2.4` incide, porque a determinação `Z1` mediu URIs distintas. `ACHADO 18`: o badge urgente e os dois cartões de urgência ganham canal **textual**, e `data-severity` não conta como canal por não ser exposto ao usuário |
| H-46 | §3.1 (Página Histórico, Performance, Clientes, Operacional, Detalhe) | ✅ **Concluída.** Fecha a parte **estática** de `R01`, `R04` e `ACHADO 21`, e trava as três em `tests/repo/estilo.test.ts` — cada guarda provada por mutação. Três tabelas passam a rolar dentro do próprio invólucro, pelo padrão que `ProcessTable` já usava: a exceção bidimensional de `SC 1.4.10` cobre a tabela, e não a página. Sete grids ganham `grid-cols-1` explícito; os quatro que já tinham base não são tocados. O `fontSize` dos eixos vira `'0.75rem'`, porque o React converte o número para pixel e pixel não acompanha a fonte-base (`SC 1.4.4`). **A verificação visual a 320 px é `VN-1`, em `H-47`** — e não é critério desta fatia |
| H-47 | §3.1 (as sete páginas), `docs/estilizacao/RESULTADO.md` | ✅ **Concluída.** Fecha a onda 7 de `E9`: cinco dos seis procedimentos de navegador percorridos em Chrome 151 por CDP, com desfecho escrito ao lado do bloco que o emitiu. **Confirma duas correções que não eram computáveis estaticamente** — `H-44`, com zero paradas de `Tab` em `aria-hidden` nas 467 percorridas, e `H-46`, com o tick do eixo indo de 12 px a 18 px quando a fonte-base vai de 16 a 24. Aprova o indicador de foco em **467 de 467** paradas e a ordem de tabulação nas sete páginas. **Cinco achados novos viraram `H-67` a `H-71`**, um por achado, porque a fatia mede e registra e não conserta. `VN-5` foi executado por emulação em seguida, e a premissa de que ele exigia Windows era falsa: quatro dos cinco alvos foram medidos, um reprovou e virou `H-72`, e o que resta é `PD-07`. Um item declarado **não exercido** — o escape de foco do `ConflictDialog` exigiria alterar a planilha real |
| H-68 | §3.1 (Página Detalhe), `docs/estilizacao/RESULTADO.md` (VN-1/B, VN-2) | ✅ **Concluída.** Fecha `VN-1/B` e a parte de `VN-2` que era o mesmo elemento: a Página Detalhe deixa de rolar horizontalmente. Medido em Chrome 151 por CDP, com o **antes reproduzido** — 572 px num viewport de 320 e 846 num de 640 com fonte-base 24 — e o depois em **305 = 305** e **625 = 625**, iguais às páginas que `H-47` aprovou. A causa não era o breakpoint tardio do `sm:max-w-sm`: era `min-width: auto` do `<select>` como item de flex resolver para a largura da maior `<option>`, e **`min-width` vencer `max-width`** — o limite não protegia nem onde incidia. Corrigido com `min-w-0` no `<label>` e `w-full` no `<select>`. O truncamento da caixa fechada não cria ambiguidade porque o nome da cor vem primeiro e os 7 prefixos são distintos |
| H-71 | §3.1 (Página Detalhe), `docs/estilizacao/RESULTADO.md` (VN-6) | ✅ **Concluída.** Fecha o achado de contraste de `VN-6`: o valor anterior de uma edição enfileirada vai de **3.23:1** a **8.73:1** sobre o painel de aviso, contra o limiar de 4.5 que `text-sm` exige. A correção é tirar `opacity-60` — **nenhum token muda**, e os três valores de `state-warning-*` foram relidos do `documentElement` para provar, o que torna o critério do token não-incidente em vez de pulado. Anterior e novo continuam distinguíveis por `line-through` e peso 700, **dois canais não cromáticos** (`ACHADO 18`). Varredura em três páginas: zero textos abaixo do limiar; as 10 setas `▾` do `MultiSelect` seguem em 4.55:1, onde `VN-6` as deixou |
| H-67 | §3.1 (Página Performance), IND-17, `docs/estilizacao/RESULTADO.md` (VN-1/A) | ✅ **Concluída.** Fecha `VN-1/A`: `/performance` vai de **385** a **305 = 305** num viewport de 320. Com `secondary` a linha do ranking tem quatro slots fixos somando 348 antes da barra, e `shrink-0` proibia o colapso. A correção **empilha** abaixo de 640 em vez de encolher — reduzir o rótulo deixaria 321, ainda rolando, e `flex-wrap` livre punha a barra em 43 px. Os dois ramos condicionam a `secondary`, que só `Performance.tsx` passa: medido no computado, os rankings sem ele ficam `nowrap` com rótulo de 160 px nas duas larguras, e a 1280 px o próprio ranking de Agentes é idêntico ao que era. As sete páginas foram medidas juntas, antes e depois. `43 atrasados` ocupa 76,0 px dos 96 do slot |
| H-69 | §3.1 (Página Performance), IND-22, `docs/estilizacao/RESULTADO.md` (VN-2/A) | ✅ **Concluída.** Fecha `VN-2/A`: o rótulo da tabela de tempo documental quebra em linhas em vez de ser cortado, e a coluna toma o espaço que as três numéricas não usam. **Zero células cortadas nos quatro cenários**, contra 31 a 100% e 41 a 640 px com fonte 24 na mesma amostra. Eliminar o corte é obrigatório, não preferência: o critério de não crescer com a ampliação é insatisfazível mantendo-o, porque o texto escala com a fonte e o container não — três variantes foram medidas e falharam. Nenhuma célula ganhou `title` nem parada de tabulação: o texto está na tela, e `title` não é revelado por teclado. Os números de `VN-2` (7 · 8 · 14) não foram reproduzidos por exigirem a planilha real; a mecânica foi recriada no DOM |
| H-72 | §3.1 (as sete páginas), `docs/estilizacao/RESULTADO.md` (VN-5) | ✅ **Concluída.** Fecha o achado de `VN-5`: sob `forced-colors: active` as sete abas tinham **uma** assinatura de borda, porque o UA pinta `border-transparent` como pinta qualquer outra — só o `aria-current` restava, e ele serve o leitor de tela e não quem enxerga. A corrente passa a **4 px** contra 2 px das outras seis, pela mesma técnica de espessura que `H-44` usou e que `VN-5` mediu sobrevivendo. A borda e a compensação de `padding` viajam na variante `forced-colors:`, então o modo normal fica idêntico ao medido antes. O `aria-current` permanece em exatamente uma das sete, e a rolagem que `H-46` tirou não volta: 305 = 305 sob modo forçado. **`PD-07` não fecha** — os itens do `ConflictDialog` e do realce de linha exigem planilha alterada e cursor real |
| H-70 | §3.1 (as sete páginas), `docs/estilizacao/RESULTADO.md` (VN-4), D-16 | ✅ **Concluída.** Fecha `VN-4` e, com ela, o épico `E9`: o foco deixa de cair no `<body>` depois de uma troca de rota programática e vai para a landmark da página nova, com o `aria-label` dela — que é o que o leitor de tela anuncia. **Mover o foco virou o padrão**, e a exceção declara: `VN-4` mediu uma origem, mas há seis navegações programáticas, e inverter o padrão em `navigate()` cobre as seis sem tocar nenhuma. O alvo é a landmark da casca porque ela existe mesmo sob o `Suspense`, o que resolve o caso-limite da rota `lazy` por construção. Zero paradas de tabulação novas nas oito rotas, e o `popstate` do botão "voltar" deixa o foco intacto. Levou `router.ts` de 79 a **97** linhas de código de ~100, e o registro de `D-16` foi atualizado |
| H-53 | IND-22, §3.1 (Página Performance) | ✅ **Concluída, com o quinto critério não-incidente.** A página passa a dizer o que mede — `RG − DOCS ENVIADOS` em dias inteiros, na ordem que A-02 fixou — e **sobre o que** mede: o painel de recorte nomeia cada filtro ativo, ou declara que os números cobrem a base inteira e aponta a barra. As duas exclusões de A-30 ganham o que significam, e a amostra zerada diz por que exibe traço. **Nenhum dos dois defeitos era de cálculo** — a métrica está certa desde IND-22 e o filtro funciona desde `H-15` —, e é isso que os tornava fáceis de deixar abertos para sempre. **O quinto critério fechou em `H-66`**, em 01/09/2026: com `H-50` entregue, a ressalva de A-31 passou a descrever de onde o responsável vem, e A-31 seguiu apontada para o filtro de cor, onde a limitação continua real |
...
| H-50 | IND-20, IND-22, §3.2 (os filtros globais), TD-05 | ✅ **Concluída.** O campo Responsável passa a nomear a **pessoa**, atribuída pelo importador contra `config/team-map.json`, com a cor desempatando o que a lista não alcança — e a cor vira campo próprio, `colorResponsible`, com filtro próprio. Medido contra a planilha real em 01/09/2026: **559** pelo importador, **48** pela cor, **42** sem responsável, **zero** divergências; a cor sozinha preenchia 165. `Responsible` deixou de ser domínio fechado, e o rótulo passou a viajar em `responsibleLabel` — o nome vive em arquivo não versionado, então nenhuma tabela no cliente o traduz. O contrato de **três** rotas mudou, e o de uma quarta não: `PATCH .../color` segue com a chave `responsible`, onde o contexto já é a cor, e nenhum `fillId` mudou. **O documento de origem afirmava 157 e o número é 165** — erro de aritmética corrigido em três documentos, como achado. **A ressalva de A-31 na Página Performance fica errada até `H-66`** |
| H-66 | IND-20, §3.2 (os filtros globais), §3.1 (Página Performance) | ✅ **Concluída, e fecha `E10`.** O filtro `colorResponsible` de `H-50` ganha controle na barra — 11 de múltipla escolha, 13 controles, 14 filtros — e a ressalva de A-31 na Página Performance passa a descrever de onde o responsável vem, em vez da limitação que `H-50` removeu. **Fecha o quinto critério de `H-53`**, que era o único critério aberto de uma história já concluída. Medido num Chrome real por `tools/medir-navegador.mjs`: nenhum estouro horizontal em 1280, 1024, 768 e 360, nem a 360 com a fonte-base em 24 px. A linha do ranking segue sem virar botão — o impedimento de A-18 acabou com `H-50`, e torná-la clicável é funcionalidade que nenhum critério pede |
| H-57 | RNF-42, §3.1 (as sete páginas) | ✅ **Concluída.** Abre `E11` com o par escuro de cada token: **44 tokens de cor**, todos com valor nos dois esquemas, mais cinco que não são cor e por isso não têm par. Medido no navegador nos dois esquemas — **74 medições de contraste, zero reprovações** —, e o controle com os hexadecimais crus do mockup reproduz as quatro reprovações que `PROPOSTA §2.2` previu, o que é o que dá crédito ao número. **Nenhum `.tsx` mudou.** `tests/repo/estilo.test.ts` passa a reprovar token de cor sem par, e token que não é cor duplicado sob a media query |
| H-59 | §3.1 (as sete páginas), `docs/redesign/PROPOSTA.md` §3, D-16 | ✅ **Concluída.** A casca troca de eixo: navegação numa coluna à esquerda, topo de **uma** linha com o nome da página, a data do dado e as duas ações. Medido em Chrome 151: sete rotas × seis larguras de 320 a 1440 px, **um `<header>` em cada, zero estouros**. **`router.ts` não foi tocado e segue em 97 linhas de código** — a separação entre destino e rodapé é apresentação, e pô-la no roteador teria disparado o gatilho de `D-16` por um dado que não é de roteamento. Revelou um defeito preexistente de `SC 1.4.10` na Página Operacional — `lg:grid-cols-[1fr_20rem]` recusa encolher —, corrigido com `minmax(0,1fr)` e agora coberto por guarda. O canal não-cromático de `H-72` migrou da borda inferior para a esquerda, medido em 4 px contra 2 px sob `forced-colors`, e entrou o salto para o conteúdo de `SC 2.4.1`, que já faltava |
| H-60 | §3.2 (os filtros globais), `docs/redesign/PROPOSTA.md` §3 | ✅ **Concluída.** Os quatorze filtros viram **treze chips** — o grupo de clientes vive dentro do de Cliente —, cada um dizendo o próprio recorte sem abrir nada. Medido em Chrome 151: a barra cai de **200 px para 83 px**, e o `<main>` sobe de 321 px, na casca de quatro faixas, para **143 px** — 178 px liberados por `H-59` e `H-60` juntas. **O critério \"uma linha\" não é atingível a 1280 px**: os treze chips somam 1437 px contra 1064 disponíveis, e cabem em uma linha a partir de ~1800 px. `VN-3` medido e limpo: nenhum ancestral recorta o popover. `Esc` passou a devolver o foco ao chip, que `MultiSelect` não fazia |
| H-58 | RNF-34, RNF-43, §3.1 (as sete páginas) | ✅ **Concluída.** IBM Plex Sans e Mono servidas do próprio repositório — seis `.woff2` e a licença OFL 1.1 em `web/public/fonts/`, **347 KB** —, com `@font-face` escrito à mão e sem dependência npm. Medido no navegador: **zero requisições externas**, `Content-Type: font/woff2`, assinatura `wOF2` íntegra, e a largura de um texto medindo 199,87 px na Plex contra 195,69 na reserva. Os dois casos-limite já eram verdade e foram medidos: **zero `font-bold`** e **zero `italic`** no conjunto. **A guarda nova pegou uma falha em si mesma**: `interfaceFiles()` só coleta `.ts`/`.tsx`, e o `@font-face` mora no CSS — a asserção de origem externa não cobria `index.css`, que é onde um CDN entraria |
| H-61 | §3.1 (Página Operacional, Inicial e Alertas), `docs/redesign/PROPOSTA.md` §1 | ✅ **Concluída.** Os seis componentes de dado adotam os dois raios, a linha de **40 px** e o mono com `tabular-nums`. Medido em Chrome 151: todas as linhas em 40 px e o contentor em 12 px — antes da correção uma célula quebrava em seis retângulos de texto e esticava a linha para **57 px**. A razão trilho/preenchimento do medidor mede **5,57** no claro e **5,89** no escuro, contra piso de 3:1. **A fatia tocou 15 arquivos, e não 6:** `C04` é guarda de conjunto, e converter o raio em 4 dos 13 do papel daria duas formas para o mesmo papel. Severidade passou a faixa lateral **mais** ícone, com o prefixo textual de `H-45` intacto — removê-lo reprova os testes de `ACHADO 18` |
