...
Auditoria de `especificacao-dashboard-comex.md`, aplicando os três cruzamentos
exigidos. Nenhum defeito foi corrigido em silêncio: cada achado tem trecho
citado, impacto e destino.
...
| **A-18** | AMBIGUIDADE | §3 | `"Bege | Processos da Samira pertencentes a outros clientes."` | Bege é subcategoria de Samira. Não se define se o "Ranking por responsável" soma bege com azul ou os separa | **RESOLVIDO**: domínio de `responsible` tem 4 valores — `samira`, `hugo`, `samira_outros_clientes`, `indefinido`. O ranking exibe os 4; o filtro "Responsável = Samira" seleciona `samira` **e** `samira_outros_clientes` |
...
| **A-47** | LACUNA / RISCO | A aba `CNPJ` contém **dado sensível**: CNPJ/CPF, inscrição estadual, `DADOS BANCÁRIOS` (26 valores), `EMAIL` (21), `EMAIL / LOGIN` (9) e **uma coluna de credencial de terceiros**, num arquivo sincronizado com o SharePoint da organização | Exposição de credenciais e dado pessoal. A especificação nunca menciona esta aba | **RESOLVIDO**: aba **excluída** do escopo — não lida, não exibida, não registrada em log. O relatório de perfilamento versionado foi sanitizado. A exposição em si antecede este projeto; fica registrada em `08-qualidade-operacao.md §4` |
...
**58 achados** — A-01 a A-58, sem lacunas na numeração — todos com destino.
Nenhum ficou sem resolução.

| Destino | Quantidade | Quais |
|---|---|---|
| RESOLVIDO por decisão de arquitetura | 52 | todos os demais |
| RESPONDIDO pelo usuário | 4 | A-11, A-38, A-46, A-48 |
