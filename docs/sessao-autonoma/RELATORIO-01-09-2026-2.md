...
> ## ⚠️ Errata — 01/09/2026, depois do fim da sessão
>
> **O dono mesclou os seis PRs e destravou duas coisas que este relatório dá
> como fechadas.** O texto abaixo permanece como foi escrito; o que mudou:
>
> | O relatório diz | O que vale agora |
> |---|---|
> | `H-58` **bloqueada** por falta dos `.woff2` (§5, e o resumo) | ✅ **Concluída** — o dono baixou os seis arquivos, e ela saiu no **PR #96**. `E11` ficou **sem história bloqueada** |
> | `H-61` a `H-65` não iniciadas por **limite de 6 PRs** | O limite foi **levantado pelo dono**. Elas seguem não iniciadas, mas por ordem de execução, não por teto |
> | Suíte em **1678** testes | **1683** — `H-58` acrescentou 5 |
> | 66 das 72 histórias | **67 das 72** |
>
> **O que segue valendo:** a **Pendência 1** — os treze chips de `H-60` somam
> 1437 px e não cabem em uma linha a 1280 px — continua **aberta e é do dono**,
> com as quatro opções e a recomendação intactas na seção 3.
>
> **Um achado que este relatório não tem, porque `H-58` veio depois dele:** a
> prova por mutação revelou falha na guarda nova de origem externa —
> `interfaceFiles()` só coleta `.ts` e `.tsx`, e o `@font-face` mora em
> `web/src/index.css`, então a guarda **não cobria o CSS**, que é onde um CDN
> entraria. Está no bloco `✅ CONCLUÍDA` de `H-58`.
>
> **Por que errata e não reescrita:** o relatório é registro do que era verdade
> quando foi escrito. Apagar "bloqueada" esconderia que a história esteve
> travada por um dia e por qual motivo — que é justamente o que a próxima sessão
> precisa saber para não repetir a tentativa. Mesmo padrão dos blocos
> `> ✅ **FECHADA em…**` do relatório de 31/08/2026.
