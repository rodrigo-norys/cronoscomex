...
**88 das 91 histórias estão concluídas**, e as três abertas são `E14`. O que cada uma aprendeu — número medido,
...
04/09/2026, e `H-87` em 08/09/2026.** Restam três: `H-88` e `H-91` — o mapa de
clientes e o de equipe operáveis pela tela, que saem de `PD-08` (`D-32`) e
crescem por `D-35` —, e a ordem em que a planilha está (`D-33`), **a única das
três que toca contrato de rota existente**, porque altera `sort` em
`docs/05-contratos-api.md`. As duas primeiras vivem na branch
`E14/clientes-editaveis` e chegam à `main` num PR só; a ordem entre elas está no
cabeçalho do épico. Restam também as **três pendências abertas** abaixo,
