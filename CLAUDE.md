...
|---|---|---|
| **PD-06** | `scripts/iniciar.cmd` foi entregue por `H-30`, **reescrito por `H-44`** e **nunca executado**: é batch do Windows (RNF-26) e o desenvolvimento é em Linux. Sem verificação, em nove itens: duplo clique · Node ausente · Node abaixo da 22 · janela fechada sem processo órfão · porta ocupada · segunda execução com a aplicação no ar · caminho com espaços e acentos · **primeira execução sem o arquivo de configuração, que agora é apenas informada e a partida segue** · **compilação sob demanda: aceitar, recusar, máquina sem internet, e `node_modules` presente e ausente**. **A primeira execução real, em 19/08/2026, derrubou dois itens do mesmo bloco.** O navegador abria depois de 4 segundos fixos e a partida demorou mais que isso — corrigido por `scripts/esperar-porta.mjs`, que pergunta à porta em vez de adivinhar. E o abridor rodava com `start /b`, **no mesmo console do servidor**: ao terminar, o evento de console derrubava o `node`, que sai com código **zero** ao tratar `SIGINT` — o atalho lia zero como término normal e fechava a janela. A aplicação morria sozinha logo após o navegador abrir, sem erro nenhum. Corrigido com `/min`, que dá janela própria ao abridor. **Nenhuma das duas foi reexecutada.** Suspeitas ainda abertas, cada uma virando correção no `.cmd`: a detecção de "já está no ar" depende do espaçamento de colunas do `netstat`; o `PATH` de uma janela aberta antes de instalar o Node não enxerga o executável novo; e o caminho do projeto com acento aparece em algumas mensagens, em code page 850. A parte servidor está testada — `GET /*` tem 11 casos e foi conferida contra o `dist/web` real | **Primeira instalação na máquina do operador.** Percorra os nove itens; cada falha é correção no `.cmd`, não história nova |
