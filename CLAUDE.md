...
|---|---|---|
| **PD-06** | `scripts/iniciar.cmd` foi entregue por `H-30`, **reescrito por `H-44`** e **nunca executado**: é batch do Windows (RNF-26) e o desenvolvimento é em Linux. Sem verificação, em nove itens: duplo clique · Node ausente · Node abaixo da 22 · janela fechada sem processo órfão · porta ocupada · segunda execução com a aplicação no ar · caminho com espaços e acentos · **primeira execução sem o arquivo de configuração, que agora é apenas informada e a partida segue** · **compilação sob demanda: aceitar, recusar, máquina sem internet, e `node_modules` presente e ausente**. **A primeira execução real, em 19/08/2026, já derrubou um item: o navegador abria depois de 4 segundos fixos, e a partida demorou mais que isso — `ERR_CONNECTION_REFUSED` com o servidor subindo atrás.** Corrigido por `scripts/esperar-porta.mjs`, que pergunta à porta em vez de adivinhar; **não reexecutado.** Suspeitas ainda abertas, cada uma virando correção no `.cmd`: a detecção de "já está no ar" depende do espaçamento de colunas do `netstat`; o `PATH` de uma janela aberta antes de instalar o Node não enxerga o executável novo; e o caminho do projeto com acento aparece em algumas mensagens, em code page 850. A parte servidor está testada — `GET /*` tem 11 casos e foi conferida contra o `dist/web` real | **Primeira instalação na máquina do operador.** Percorra os nove itens; cada falha é correção no `.cmd`, não história nova |
